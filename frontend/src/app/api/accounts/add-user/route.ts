...
export async function POST(req: NextRequest) {
  try {
    ...
    const { qcode, user_icodes, access_level } = body;
    
    if (!qcode || !Array.isArray(user_icodes) || user_icodes.length === 0) {
      return new NextResponse("qcode and user_icodes[] are required", { status: 400 });
    }
    ...
    const results = await Promise.all(
      user_icodes.map(icode =>
        prisma.pooledAccountUser.create({
          data: {
            qcode: qcode,  // Changed from account_id to qcode
