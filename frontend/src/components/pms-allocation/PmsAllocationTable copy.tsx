...
export default function PmsAllocationTable() {
    ...
    const [newCustodianCodes, setNewCustodianCodes] = useState<string[]>([]);
    ...
    const uniqueCustodianCodes = useMemo(() => {
        ...
        return [...newCustodianCodes, ...originalCodes];
    ...
    const totalRupeesByCustodian = useMemo(() => {
        const totals = uniqueCustodianCodes.reduce((acc, custodianCode) => {
            if (newCustodianCodes.includes(custodianCode)) {
                acc[custodianCode] = parseFloat(newClientAmount) || 0;
            } else {
        ...
        return totals;
    }, [uniqueCustodianCodes, filteredAllocations, newCustodianCodes, newClientAmount]);
    ...
    const summaryData = useMemo(() => {
        ...
        // Count number of stocks in the Cash category
        const cashStocks = Object.entries(groups)
            .filter(([_, items]) => items[0].asset_class === "Cash")
            .map(([key]) => key);
        const cashStockCount = cashStocks.length;
        const cashItems = filteredAllocations.filter((item) => item.asset_class === "Cash");
        const cashStockKeys = _.uniq(cashItems.map((item) => item.stock_name));
        const cashAllocationPerStock = cashStockKeys.length > 0 ? 100 / cashStockKeys.length : 0;
        ...
        return Object.entries(groups).map(([key, items]) => {
            ...
            uniqueCustodianCodes.forEach((custodianCode) => {
                if (newCustodianCodes.includes(custodianCode)) {
                    const isCashStock = details.asset_class === "Cash" && cashStockKeys.includes(key);
                    custodianCodeAllocations[custodianCode] = isCashStock ? cashAllocationPerStock.toFixed(2) : "0.00";
                    initialCustodianAllocations[custodianCode] = isCashStock ? cashAllocationPerStock.toFixed(2) : "0.00";
                } else {
                    ...
                    items.forEach((item) => {
                        if (item.custodian_code === custodianCode) {
                            ...
                            initialCustodianAllocations[custodianCode] = percentValue;
                        }
                    });
                }
            });
    ...
    const getClientDifference = useCallback(
        (item: Summary, custodianCode: string) => {
            if (newCustodianCodes.includes(custodianCode)) {
                ...
                return item.asset_class === "Cash" ? modelPercent - 100 : modelPercent;
            }
    ...
    const exportToExcel = useCallback(() => {
        ...
        uniqueCustodianCodes.forEach((custodianCode, rowIndex) => {
            ...
            stockKeys.forEach((stockKey, colIndex) => {
                if (!selectedClients.includes(custodianCode)) {
                    row.push("-");
                    return;
                }
    ...
    const handleAddNewClient = useCallback(() => {
        ...
        setNewClientError(false);
        const newCustodianCode = `NEW_CLIENT_${newCustodianCodes.length + 1}`;
        setNewCustodianCodes((prev) => [newCustodianCode, ...prev]);
        setSelectedClients((prev) => [newCustodianCode, ...prev]);
        setNewClientAmount("");
    }, [newClientAmount, newCustodianCodes]);
                ...
                <div className="space-y-6 font-inter">
                    ...
                    <div className="bg-gray-50 p-4 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
                            ...
                            <button
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 shadow-sm flex items-center gap-2 ${newClientAmount === "" || parseFloat(newClientAmount) <= 0
                                    ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                                    : "bg-blue-600 text-white hover:bg-blue-700"
                                    }`}
                                onClick={handleAddNewClient}
                                disabled={newClientAmount === "" || parseFloat(newClientAmount) <= 0}
                            >
                    ...
                    <div className="relative overflow-x-auto rounded-xl shadow-sm border border-gray-100">
                        <table className="min-w-full text-sm bg-white">
                            <thead className="bg-gray-100 text-gray-600 uppercase font-semibold text-xs">
                                <tr>
                                    <th className="p-3 text-center w-[120px] sticky top-0 bg-gray-100 z-10 border-r border-gray-200">
                                        Actions
                                    </th>
                                ...
                                {sortedSummaryData.length === 0 ? (
                                    ...
                                    <>
                                                        ...
                                                        {uniqueCustodianCodes.map((custodianCode) => (
                                                            <td
                                                                ...
                                                                className={`p-3 text-right font-semibold border-r border-gray-200 ${selectedClients.includes(custodianCode)
                                                                    ? tab === "change"
                                                                        ? calculateTotals(assetClassItems).clientTotals[custodianCode] >
                                                                            bufferValue
                                                                            ? "text-blue-500"
                                                                            : calculateTotals(assetClassItems).clientTotals[custodianCode] <
                                                                                -bufferValue
                                                    ...
                                                    {openAccordions[assetClass] &&
                                                        assetClassItems.map((item, index) => {
                                                            ...
                                                            return (
                                                                ...
                                                                >
                                                                    <td className="p-3 text-center sticky left-0 z-10 bg-inherit border-r border-gray-200">
                                                                        {tab === "change" ? (
                                        ...
                                        <tr className="bg-gray-200 font-bold text-gray-800">
                                            <td className="p-3 text-center sticky left-0 bg-gray-200 z-10 border-r border-gray-200">
                                            </td>
                                            ...
                                            {uniqueCustodianCodes.map((custodianCode) => (
                                                <td
                                                    ...
                                                    className={`p-3 text-right font-bold border-r border-gray-200 ${selectedClients.includes(custodianCode)
                                                        ? tab === "change"
                                                            ? calculateTotals(sortedSummaryData, true).clientTotals[
                                                                custodianCode
                                                            ] > bufferValue
                                                                ? "text-blue-500"
                                                                : calculateTotals(sortedSummaryData, true).clientTotals[
