...
export default function PmsAllocationTable() {
    ...
    const [mostRecentClient, setMostRecentClient] = useState<string | null>(null);
    ...
    const uniqueCustodianCodes = useMemo(() => {
        ...
        if (mostRecentClient && codes.includes(mostRecentClient)) {
            return [
                mostRecentClient,
                ...codes
                    .filter((code) => code !== mostRecentClient)
                    .sort((a, b) => {
                        // Prioritize selectedClients, then sort alphabetically
    ...
    }, [strategyFilteredAllocations, selectedClients, mostRecentClient]);
    ...
    const getClientDifference = useCallback(
        (item: Summary, custodianCode: string) => {
            const current = parseFloat(
                adjustedAllocations[item.key]?.[custodianCode]?.percent?.toString() ??
                item.custodianCodeAllocations[custodianCode]?.toString() ?? "0"
            );
    ...
    const handleAddClient = useCallback(() => {
        ...
        const cashStock = summaryData.find((item) => item.key === "Cash")?.key
            // …otherwise just pick the first one in the Cash asset class
            || summaryData.find((item) => item.asset_class === "Cash")?.key
            || "Cash";
        ...
        setAdjustedAllocations((prev) => {
            ...
            summaryData.forEach((item) => {
                ...
                newAdjusted[item.key][newClientName] = {
                    percent: item.key === cashStock ? 100 : 0,
                    rupees: item.key === cashStock ? cashAmount : 0,
                };
            });
            return newAdjusted;
        });
        ...
        // Reset input fields
        setMostRecentClient(newClientName);
        setNewClientName("");
        setNewClientCash("");
    }, [
        newClientName,
