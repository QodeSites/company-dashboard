...
export default function MutualFundHoldingSyncComponent() {
  ...
  const fetchLatestSyncs = async () => {
    try {
      const response = await fetch("/api/mutual-fund-holding-sync-history");
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/mutual-fund-holding-sync", {
        method: "POST",
        headers: {
      ...
      window.dispatchEvent(new CustomEvent('mutualFundSyncCompleted'));
  ...
  if (isLoading) {
    return (
      <ComponentCard title="Mutual Fund Holding Sync" className="p-6">
